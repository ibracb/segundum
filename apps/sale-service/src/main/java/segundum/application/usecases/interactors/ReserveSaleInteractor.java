...
/**
 * Represents the interactor for reserving a sale by the seller.
 */
public class ReserveSaleInteractor implements ReserveSaleUseCase {
    ...
    /**
     * Constructs a new ReserveSaleInteractor with the given dependencies.
     *
     * @param eventStore     the event store for loading and appending domain events
     * @param eventPublisher the domain event publisher
     */
    public ReserveSaleInteractor(EventStore eventStore, DomainEventPublisher eventPublisher) {
        this.eventStore = eventStore;
        this.eventPublisher = eventPublisher;
    }

    @Override
    public void execute(ReserveSaleCommand command) {
        ...
        sale.reserve(command.getSellerId());
